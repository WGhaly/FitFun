const { Database, initDatabase } = require('../lib/database');
...
// Create super admin
const superAdmin = Database.createAdmin({
...
const competition = Database.createCompetition({
...
// Add participants
Database.addParticipant(competition.id, user1.id, true);
Database.addParticipant(competition.id, user2.id, true);
console.log('✅ Added participants to competition');

// Create testimonials
Database.createTestimonial({
    user_id: user1.id,
...
const approvedTest = Database.createTestimonial({
    user_id: user2.id,
    competition_id: competition.id,
...
Database.updateTestimonial(approvedTest.id, {
    status: 'approved',
